...
class Evaluator:
    ...
    def __init__(self, conf):
        ...
        logging.basicConfig(filename=os.path.join(conf['log_dir'], 'log.txt'),
    ...
    def print_info(self):
        ...
        logging.info('OA:{:.4f}\t AA:{:.4f}\t Ka:{:.4f}\t F1:{:.4f}'.format(OA, AA, Kappa, np.nanmean(F1_score)))

        logging.info('{:<25}{:<10}{:<10}{:<10}'.format('Everyclass', 'PC', 'RC', 'F1'))
        for ind_class in range(self.num_classes):
            ...
            logging.info('{:<25}{:<10}{:<10}{:<10}'.format(self.class_names[ind_class], pc, rc, f1))
